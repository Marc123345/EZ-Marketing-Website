...
- Home hero 3D scene: `src/hero3d/Hero3D.tsx` (the H2H Shannon Hero3D, unchanged motion, tokens rendered as rounded cubes) mounted by `src/hero3d/index.tsx`, bundled with `npm i && npm run build:hero` into `src/assets/js/hero3d.js` (commit the bundle; Vercel does not install packages)
